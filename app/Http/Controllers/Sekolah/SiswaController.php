...
class SiswaController extends Controller
{
    ...
    public function import(Request $request): RedirectResponse
    {
        ...
        if (! in_array('nama_siswa', $header) || ! in_array('nisn', $header)) {
            ...
            return redirect()->back()->with('error', 'File CSV harus memiliki kolom nama (atau nama_siswa) dan nisn.');
        }
        ...
        while (($row = fgetcsv($handle, 1000, $delimiter)) !== false) {
            ...
            $status = 'Active';
            if (strtolower($statusRaw) === 'inactive' || strtolower($statusRaw) === 'tidak aktif' || $statusRaw === 'Inactive') {
                $status = 'Inactive';
            }
